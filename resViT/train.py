...
def train_pr(Model, TrainLoader, ValLoader, optC, criterion, tr_loss = [], val_loss = [], num_epochs=20):
    mmD = pr2np.adaptACC(num_epochs, len(TrainLoader), len(ValLoader))
    mmd = torch.from_numpy(np.array(mmD))
    ...
    for epoch in range(num_epochs):
        ...
        #       ClassifierLoss = criterion(predictions, labels) + mmd
        ...
        train_acc = pr2np.CompInf(epoch+1, correct_tr, total_tr, 0)#(correct_tr / total_tr) * 100
        ...
        val_acc = pr2np.CompInf(epoch+1, correct_val, total_val, 1) #(correct_val / total_val) * 100
